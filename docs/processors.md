...
| 5 | **Sentry (Functional Software Inc.)** | **EU (de.sentry.io)** — Team plan since 2026-04-24 | Backend + frontend error tracking (**two projects**: `python-fastapi` for backend+workers, `hedgespark-frontend` for dashboard), session replay (merchant text + media masked), cron monitoring, profiling | Stack traces with shop_domain + request_id — `send_default_pii=False` enforced in `app/core/sentry_init.py` + runtime `before_send` scrub via `app/core/llm_pii_guard.sanitize` redacts emails / API keys / bearer tokens / IBAN / phone / Shopify tokens across exception values, breadcrumb messages, capture_message text, request bodies. Replay masks all text + media (`maskAllText=true`, `blockAllMedia=true`). | Sentry DPA | Yes | ✅ In place (config) / ⚠️ Verify signed copy |
