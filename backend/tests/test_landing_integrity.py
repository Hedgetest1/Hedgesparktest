...
def test_landing_css_references_are_all_reachable():
    ...
    Note: Next 16.2+ uses alphanumeric+underscore+tilde filenames like
    `0neevhl_o1ozu.css` and `0~e~w2kyvoghc.css` instead of pure hex,
    so the regex character class allows all three special chars. (Bug
    history: an earlier regex omitted `~`, which made the audit miss
    the largest CSS chunk and falsely triggered "tailwind purged
