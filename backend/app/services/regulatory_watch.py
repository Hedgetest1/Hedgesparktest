...
def run_regulatory_audit(db: Session) -> dict[str, Any]:
    ...
    for rule in REGULATORY_RULES:
        ...
        # check_fn is kept OUTSIDE the savepoint: every _check_* is a
        # read-only file/import probe (independent-Agent-verified
        # a42ab1c29c0a22727 — zero db touch), so a raising check = the
        # rule is non-compliant, NOT a poisoned session.
        try:
        ...
        # Compliance-status accounting is INDEPENDENT of the alert DB
        # side-effect — recorded UNCONDITIONALLY here, exactly as the
        # pre-2026-05-19 original did (it incremented passed / failed+
        # gap BEFORE the dedup/alert write). A transient DB error on
        # the alert write must NEVER silently erase a real regulatory
        # gap from the report (§0 truth). Agent-caught drift
        # (ad4f573b72a2a02a1 Finding 2): folding these AFTER the
        # savepoint dropped a known gap on any savepoint raise.
        if compliant:
            report["passed"] += 1
        else:
            report["failed"] += 1
        ...
        # The alert DB side-effect runs in this rule's OWN SAVEPOINT
        # (write_no_rollback class close 2026-05-19c, Agent-verified
        # a42ab1c…+ad4f573b…). BATCH loop (single post-loop commit
        # ~line 727). CRITICAL INVARIANT: NO inner try/except may
        # swallow a DB error in here — a poisoned op MUST propagate to
        # savepoint_scope's exception branch (rolls back ONLY this
        # rule's savepoint; session clean for the next rule). Finding-1
        # root-fixed separately: write_audit_log's anchor write is now
            ...
            with savepoint_scope(db):
                ...
                if compliant:
                    if existing is not None:
                        ...
                        db.flush()
                        rule_outcome = "auto_resolved"
                    else:
                        rule_outcome = "passed"
                elif existing is not None:
                    rule_outcome = "deduped"  # already alerted
                else:
                    alert = OpsAlert(
                    ...
                    rule_outcome = "new_alert"
            # Savepoint released cleanly → fold the persistence-
            # contingent counters: `auto_resolved`/`new_alerts` count
            # ONLY work that actually committed. (passed/failed/gaps
            # were recorded unconditionally above — exact original.)
            #
            # INTENTIONAL §0 CORRECTION (not exact-original — disclosed,
            # Agent-flagged ab6d901947b397f8a Finding 2-residual): the
            # HEAD original incremented new_alerts BEFORE write_audit_
            # log, then on a write_audit_log failure did a full
            # db.rollback() that discarded the OpsAlert (and every prior
            # rule's uncommitted work) WHILE leaving new_alerts=+1 — a
            # report claiming an alert its own rollback destroyed (§0
            # false claim) + a cascade. v2 deliberately diverges:
            # new_alerts counts an alert ONLY once its savepoint
            # releases (⟹ it will be persisted by the post-loop
            # commit). A write_audit_log raise rolls back ONLY this
            # rule's savepoint and correctly does NOT claim a new alert.
            # Truthful > bug-compatible. Locked by
            # test_regulatory_watch_audit_log_raise_no_phantom_alert.
            if compliant and rule_outcome == "auto_resolved":
                report["auto_resolved"] += 1
                log.info(
                    "regulatory_watch: auto-resolved %s (now compliant)",
                    rule.rule_id,
                )
            elif (not compliant) and rule_outcome == "new_alert":
                report["new_alerts"] += 1
        except Exception as exc:
            # Savepoint rolled back this rule's alert side-effect; the
            # compliance gap/pass is ALREADY recorded above so it is
            # never lost. Session is clean for the next rule.
            log.warning(
                "regulatory_watch: rule %s alert side-effect isolated "
                "(savepoint rolled back): %s",
                rule.rule_id, exc,
            )
            continue
