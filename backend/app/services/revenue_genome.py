    ...
    try:
        ...
        genes["customer"] = {
            ...
            "genes": [
                ...
                _gene("aov", _score(avg_aov, 20, 150), round(avg_aov, 2), currency or "USD",
                      f"Average order value: {currency or 'USD'} {avg_aov:.0f}.",
                      "Increase AOV with bundles, upsells, or free shipping thresholds." if avg_aov < 50 else "AOV is healthy."),
                _gene("revenue_per_customer", _score(rpc, 30, 300), round(rpc, 2), f"{currency or 'USD'}/90d",
                      f"Revenue per customer (90d): {currency or 'USD'} {rpc:.0f}.",
