...
| `hs:aggregation:cursor` | aggregation_worker store_metrics **COLD-tier** round-robin cursor (J1 tiered cadence 2026-05-17: integer position into the sorted COLD list = active merchants with NO events this cycle; HOT = shops_processed_set is processed every cycle untracked. Advance-by-cold-actually-processed. `app/workers/_rr_cursor.py`, degrade-open) | 24h |
...
| `hs:ingest:buf` | J3-part-2 async analytics-event buffer (Redis LIST). Non-purchase storefront events RPUSHed by `/track` (ZERO request DB conn) + atomic `LPOP count` drained by the singleton `ingest-drain` thread in aggregation_worker → batched `execute_values` bulk-INSERT into `events`. Length-capped `INGEST_BUFFER_MAX` (drop-oldest LTRIM on overflow — fire-and-forget analytics, bounded RAM). Purchases are NEVER buffered (revenue stays synchronous, §0). No TTL by design (drained continuously; cap bounds it) | n/a (length-capped, continuously drained) |
| `hs:ingest:cb` | J3-part-1 storefront-ingest admission ZSET (member=token, score=epoch). Global concurrent-expensive-ingest cap (`INGEST_ADMIT_BUDGET`, default 30 ≪ PgBouncer pool 80 — SHARED with reads+workers, must leave headroom) so a 10k aggregate event storm can't pool-cascade; over-cap → fast 429 (tracker is fire-and-forget, bounded shed is graceful not data-loss). Stale entries (crashed) self-purge >10s. Mirror of `hs:dash:cb`; degrade-open with per-worker BoundedSemaphore floor. `app/core/ingest_admission.py` | ~15s (sliding, ZSET self-prunes) |
| `hs:brain:cursor` | Brain Vero `tick_all_active_merchants` round-robin cursor (integer position into the sorted active-merchant list; advance-by-actual-processed). Sibling of `hs:aggregation:cursor` — born 2026-05-17 closing the 1%-coverage cliff (old `.limit(100)` no-order returned the same arbitrary merchants every 15min cycle → 99% of merchants never brain-ticked at 10k). `app/workers/_rr_cursor.py` shared helper, degrade-open | 24h |
| `hs:billing_sync:cursor` | `billing_sync.run_billing_sync` round-robin cursor (integer position into the sorted billable-Pro-merchant `shop_domain` list; advance-by-actual-processed). Sibling of `hs:brain:cursor` — born 2026-05-18, surfaced by the extended `audit_worker_loop_cursor` (a real §11 sibling of the merchant_brain miss): old `.limit(_MAX_PER_CYCLE)` no-order re-checked the same arbitrary Pro merchants every 15min agent_worker cycle → at 10k ~all other Pro merchants never billing-verified → a merchant who cancelled in Shopify keeps Pro indefinitely (revenue/entitlement integrity). `app/workers/_rr_cursor.py` shared helper, degrade-open; rr_slice returns the whole list at n≤_MAX_PER_CYCLE ⟹ zero behaviour change below scale | 24h |
